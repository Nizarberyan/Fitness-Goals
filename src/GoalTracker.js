...
class GoalTracker extends Component {
  ...
  addGoal = async () => {
    ...
    if (newGoal.trim() !== "") {
      try {
        ...
        this.setState({
          goals: [...this.state.goals, response.data],
          newGoal: "",
        });
      } catch (error) {
  ...
  render() {
    const { goals, newGoal, isModalOpen, selectedGoalIndex, newProgress } =
      this.state;
    const { completedGoalsCount, averageProgress } =
      ...
      <div className="max-w-4xl mx-auto p-10 bg-gradient-to-br from-indigo-50 via-blue-50 to-purple-50 rounded-2xl shadow-xl border border-indigo-100">
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg mb-8">
          ...
          <div className="text-sm text-gray-600">
            <p>
              <strong>Goals Completed:</strong> {completedGoalsCount}
            </p>
            <p>
        ...
        <div className="space-y-5">
          {goals.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-xl shadow-sm border border-gray-100">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-16 w-16 mx-auto text-indigo-200 mb-4"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
              ...
              <p className="text-gray-500 text-lg">
                No goals yet. Add your first fitness goal above!
              </p>
            </div>
          ) : (
            goals.map((goal, index) => (
              ...
              >
                <div className="flex justify-between items-center mb-3">
                  <h4
                    className={`font-semibold text-xl ${goal.progress >= 100 ? "text-green-700" : "text-gray-800"}`}
                  >
                    {goal.name}
                    {goal.progress >= 100 && (
